...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int elapsedDays = 0;
        if(!doesTomatoCanRipe()) {
            System.out.println(-1);
            return;
        }

        while(true) {
            if(isEveryTomatoRipe()) {
    ...
    private static boolean doesTomatoCanRipe() {
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < n; j++) {
                for (int k = 0; k < m; k++) {
                    if(warehouse[i][j][k] == 0 && !doesTomatoCanRipe(i, j, k)) {
                        return false;
                    }
                }
            }
    ...
    private static boolean doesTomatoCanRipe(int z, int x, int y) {
        for (int i = 0; i < 6; i++) {
            int nz = z + dz[i];
            int nx = x + dx[i];
            int ny = y + dy[i];
    ...
    private static void ripeTomatoes() {
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < n; j++) {
                for (int k = 0; k < m; k++) {
                    if(warehouse[i][j][k] == 1 && !visited[i][j][k]) {
                        bfs(i, j, k);
                    }
                }
            }
